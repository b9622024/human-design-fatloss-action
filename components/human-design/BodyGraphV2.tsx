...
type Shape = {kind:"polygon";points:Point[]} | {kind:"rect";x:number;y:number;width:number;height:number;rx:number};
...
const GATE:Record<number,Point> = {
  ...
  1:{x:450,y:352},7:{x:423,y:379},13:{x:477,y:379},10:{x:408,y:394},25:{x:492,y:394},15:{x:435,y:421},46:{x:465,y:421},2:{x:450,y:436},21:{x:534,y:354},51:{x:523,y:394},26:{x:525,y:412},40:{x:560,y:412},
  48:{x:309,y:468},57:{x:337,y:483},44:{x:365,y:497},50:{x:374.4,y:518},32:{x:337.5,y:537},18:{x:298.6,y:557},28:{x:259.7,y:577},
  37:{x:528,y:501},22:{x:565,y:482},36:{x:603,y:462},6:{x:525.6,y:518},49:{x:562.5,y:537},55:{x:601.4,y:557},30:{x:640.3,y:577},
  5:{x:426,y:492},14:{x:450,y:492},29:{x:474,y:492},34:{x:408,y:510},27:{x:408,y:536},59:{x:408,y:562},3:{x:426,y:580},9:{x:450,y:580},42:{x:474,y:580},
  60:{x:424,y:646},52:{x:450,y:646},53:{x:476,y:646},54:{x:397,y:666},58:{x:397,y:689},38:{x:397,y:712},19:{x:503,y:666},39:{x:503,y:689},41:{x:503,y:712},
};
...
function RoutedChannel({points,aSource,bSource,active}:{points:Point[];aSource:GateSource;bSource:GateSource;active:boolean}){const split=midpointIndex(points);const first=points.slice(0,split+1);const second=points.slice(split);if(active){return <g>{aSource!=="inactive"&&<polyline points={pointsAttr(first)} fill="none" stroke={sourceColor(aSource)} strokeWidth="5.4" strokeLinecap="butt" strokeLinejoin="miter"/>}{bSource!=="inactive"&&<polyline points={pointsAttr(second)} fill="none" stroke={sourceColor(bSource)} strokeWidth="5.4" strokeLinecap="butt" strokeLinejoin="miter"/>}</g>;}const aEnd=first.length>1?first.slice(0,2):first;const bEnd=second.length>1?second.slice(-2):second;return <g>{aSource!=="inactive"&&<polyline points={pointsAttr(aEnd)} fill="none" stroke={sourceColor(aSource)} strokeWidth="5.1" strokeLinecap="butt"/>}{bSource!=="inactive"&&<polyline points={pointsAttr(bEnd)} fill="none" stroke={sourceColor(bSource)} strokeWidth="5.1" strokeLinecap="butt"/>}</g>;}
function ChannelActivation({a,b,aSource,bSource,active}:{a:Point;b:Point;aSource:GateSource;bSource:GateSource;active:boolean}){if(active){const m=lerp(a,b,.5);return <g>{aSource!=="inactive"&&<line x1={a.x} y1={a.y} x2={m.x} y2={m.y} stroke={sourceColor(aSource)} strokeWidth="5.4" strokeLinecap="butt"/>}{bSource!=="inactive"&&<line x1={m.x} y1={m.y} x2={b.x} y2={b.y} stroke={sourceColor(bSource)} strokeWidth="5.4" strokeLinecap="butt"/>}</g>;}const aEnd=lerp(a,b,.24),bStart=lerp(a,b,.76);return <g>{aSource!=="inactive"&&<line x1={a.x} y1={a.y} x2={aEnd.x} y2={aEnd.y} stroke={sourceColor(aSource)} strokeWidth="5.1" strokeLinecap="butt"/>}{bSource!=="inactive"&&<line x1={bStart.x} y1={b.y+(a.y-b.y)*.24} x2={b.x} y2={b.y} stroke={sourceColor(bSource)} strokeWidth="5.1" strokeLinecap="butt"/>}</g>;}
function ActivationPanel({x,title,color,activations,align}:{x:number;title:string;color:string;activations:HumanDesignActivation[];align:"left"|"right"}){return <g><text x={x} y="38" textAnchor={align==="left"?"start":"end"} fontSize="15" fontWeight="800" fill={color}>{title}</text>{activations.map((a,i)=>{const y=63+i*23;return <g key={`${title}-${a.body}`}><text x={x} y={y} textAnchor={align==="left"?"start":"end"} fontSize="13" fontWeight="700" fill={color}>{BODY_SYMBOL[a.body]??"•"}</text><text x={x+(align==="left"?20:-20)} y={y} textAnchor={align==="left"?"start":"end"} fontSize="12" fontWeight="700" fill="#24232e">{a.gate}.{a.line}</text></g>;})}</g>;}
export function BodyGraph({chart,personalityActivations=[],designActivations=[],width=900}:Props){
  const defined=new Set(chart.centers);const activeChannels=new Set(chart.channels.map(id=>{const[a,b]=id.split("-").map(Number);return canonical(a,b);}));const personality=new Set(personalityActivations.map(a=>a.gate));const design=new Set(designActivations.map(a=>a.gate));
